...
import { getClinicSettings as getDemoClinicSettings, getDatabase, listDoctorAvailabilityByDoctor, replaceDoctorAvailability, updatePatientProfileAccount } from './local-db';
...
type DoctorAvailabilityRow = Database['public']['Tables']['doctor_availability']['Row'];
...
async function invokeSupabaseFunction<T>(name: string, body: Record<string, unknown>) {
  ...
  if (error) {
    if (error instanceof FunctionsHttpError) {
      ...
      throw new Error(rawText || 'Edge Function returned a non-2xx status code.');
    }
...
export function mapPatient(row: PatientRow): Patient {
  return {
    ...
    userId: row.user_id,
    qrCode: row.qr_code,
    intakeSource: row.intake_source === 'staff_walk_in' ? 'staff_walk_in' : 'online_registration',
    visitStatus: row.visit_status === 'visited_clinic' ? 'visited_clinic' : 'registered_no_visit',
...
function mapBookingPaymentStatus(value: string | null | undefined): BookingPaymentStatus {
...
export async function listPatientsLiveOrDemo() {
  ...
  const client = requireSupabase();
  const { data, error } = await client.from('patients').select('*').is('deleted_at', null).order('created_at', { ascending: false });
  if (error) {
    throw error;
  }
...
export async function createPatientLiveOrDemo(input: Omit<Patient, 'id' | 'createdAt' | 'updatedAt'>) {
  if (!isSupabaseConfigured) {
...
export async function listAppointmentsByPatientIdLiveOrDemo(patientId: string): Promise<Appointment[]> {
  if (!isSupabaseConfigured) {
    return getDatabase().appointments.filter((appointment) => appointment.patientId === patientId);
  }
...
export async function listConsultationsByPatientIdLiveOrDemo(patientId: string): Promise<Consultation[]> {
  if (!isSupabaseConfigured) {
    return getDatabase().consultations.filter((consultation) => consultation.patientId === patientId);
  }
...
export async function listPrescriptionsByPatientIdLiveOrDemo(patientId: string): Promise<Prescription[]> {
  if (!isSupabaseConfigured) {
    return getDatabase().prescriptions.filter((prescription) => prescription.patientId === patientId);
  }
...
export async function createConsultationLiveOrDemo(input: Omit<Consultation, 'id' | 'createdAt' | 'updatedAt'>) {
  if (!isSupabaseConfigured) {
  ...
  const { data, error } = await client.from('consultations').insert(payload as never).select('*').single();
  if (error) {
    throw error;
  }
...
export async function createPrescriptionLiveOrDemo(input: Omit<Prescription, 'id' | 'createdAt' | 'updatedAt'>) {
  if (!isSupabaseConfigured) {
  ...
  const { data, error } = await client.from('prescriptions').insert(payload as never).select('*').single();
  if (error) {
    throw error;
  }
...
export async function listServicesLiveOrDemo() {
  ...
  const client = requireSupabase();
  const { data, error } = await client.from('services').select('*').is('deleted_at', null).order('name');
  if (error) {
    throw error;
  }
...
export async function createServiceLiveOrDemo(input: Omit<Service, 'id' | 'createdAt' | 'updatedAt' | 'deletedAt'>) {
  if (!isSupabaseConfigured) {
  ...
  const { data, error } = await client.from('services').insert(payload as never).select('*').single();
  if (error) {
    throw error;
  }
...
export async function listSpecialtiesLiveOrDemo() {
  ...
  const client = requireSupabase();
  const { data, error } = await client.from('specialties').select('*').is('deleted_at', null).order('name');
  if (error) {
    throw error;
  }
...
export async function createSpecialtyLiveOrDemo(input: Omit<Specialty, 'id' | 'createdAt' | 'updatedAt' | 'deletedAt'>) {
  if (!isSupabaseConfigured) {
  ...
  const { data, error } = await client.from('specialties').insert(payload as never).select('*').single();
  if (error) {
    throw error;
  }
...
export async function getDoctorDirectoryLiveOrDemo(): Promise<DoctorDirectoryItem[]> {
  ...
  const client = requireSupabase();
    const { data, error } = await client
      .from('doctors')
      .select('id, profile_id, specialty_id, consultation_fee, follow_up_fee, profiles!inner(full_name), specialties(name)')
    .is('deleted_at', null)
    .order('created_at');
...
export async function getCurrentDoctor(userId: string) {
  if (!isSupabaseConfigured) {
    const user = getDatabase().users.find((item) => item.id === userId || item.authUserId === userId);
  ...
  const client = requireSupabase();
  const { data, error } = await client.from('doctors').select('*').eq('profile_id', userId).maybeSingle();
  if (error) {
    throw error;
  }
...
export async function ensureDoctorForUser(user: User) {
  const metadataRole = mapRole((user.user_metadata as Record<string, string | undefined>).role);
  ...
  const { error } = await client.from('doctors').insert({ profile_id: user.id } as never);
  if (error) {
    throw error;
  }
...
export async function getDoctorAvailabilityByDoctorIdLiveOrDemo(doctorId: string | null) {
  if (!doctorId) {
    return [];
  }
...
export async function saveDoctorAvailabilityForProfileLiveOrDemo(
  profileId: string,
  availability: Array<Omit<DoctorAvailability, 'id' | 'createdAt' | 'updatedAt'>>,
) {
  ...
  const client = requireSupabase();
  const { error: deleteError } = await client.from('doctor_availability').delete().eq('doctor_id', doctor.id);
  if (deleteError) {
    throw deleteError;
  }
  ...
  const { data, error } = await client.from('doctor_availability').insert(payload as never).select('*');
  if (error) {
    throw error;
  }
...
export async function saveDoctorFeeSettingsForProfileLiveOrDemo(profileId: string, input: DoctorFeeSettings) {
  const doctor = await getCurrentDoctor(profileId);
  if (!doctor) {
    throw new Error('Doctor record not found for this profile.');
  }
...
export async function listBlockedBookingSlotsLiveOrDemo(input: { date: string; doctorId?: string | null; serviceId?: string | null }) {
  if (!input.date) {
    return [];
  }
  ...
  if (!isSupabaseConfigured) {
    ...
    const bookingTimes = database.bookings
      .filter((booking) => {
        if (booking.preferredDate !== input.date || booking.status === 'cancelled') {
          return false;
        }
    ...
    const appointmentTimes = database.appointments
      .filter((appointment) => {
        if (!appointment.scheduledAt.startsWith(input.date) || appointment.status === 'cancelled') {
          return false;
        }
        ...
        return appointment.serviceId === input.serviceId && !appointment.doctorId;
      })
  ...
  const { data, error } = await (client as unknown as {
    rpc: (
      fn: string,
      args: { booking_date: string; booking_doctor_id: string | null; booking_service_id: string | null },
    ) => Promise<{ data: unknown; error: Error | null }>;
  }).rpc('list_blocked_booking_slots', {
  ...
  return ((data ?? []) as Array<{ blocked_time: string }>).map((row) => row.blocked_time.slice(0, 5));
}
...
export async function getBookingByReceiptCodeLiveOrDemo(receiptCode: string) {
  ...
  const client = requireSupabase();
  const { data, error } = await client.from('bookings').select('*').eq('receipt_code', receiptCode).maybeSingle();
  if (error) {
    throw error;
  }
...
export async function markBookingPaidAndCreateInvoiceLiveOrDemo(receiptCode: string) {
  if (!isSupabaseConfigured) {
  ...
  const client = requireSupabase();
  const { data: bookingRow, error: bookingError } = await client.from('bookings').select('*').eq('receipt_code', receiptCode).single();
  if (bookingError) {
    throw bookingError;
  }
  ...
  if (booking.payment_status !== 'paid') {
    ...
    const { data: createdInvoiceRow, error: invoiceError } = await client.from('invoices').insert(invoicePayload as never).select('*').single();
    if (invoiceError) {
      throw invoiceError;
    }
    const createdInvoice = createdInvoiceRow as { id: string; invoice_number?: string; total?: number };
    invoice = createdInvoice;

    const services = await getBookableServicesLiveOrDemo();
    const serviceName = services.find((service) => service.id === booking.service_id)?.name ?? 'Medical Service';
    const lineDescription =
      booking.fee_type === 'follow_up'
        ...
        : booking.fee_type === 'consultation'
          ? 'Consultation Fee'
          : serviceName;
    const category = booking.fee_type === 'service_fee' ? 'other' : 'consultation';
...
export async function createAdminUserLiveOrDemo(input: AdminCreateUserInput) {
  if (!isSupabaseConfigured) {
    ...
    return createUserProfile({
      ...
      title: null,
      specialtyId: null,
      consultationFee: input.role === 'doctor' ? input.consultationFee ?? 0 : null,
  ...
  const data = await invokeSupabaseFunction<AdminCreateUserResponse>('admin-create-user', payload);
  if (!data.user) {
