...
export function PatientRegisterForm() {
  ...
  const availableMonthOptions = monthOptions.filter(
    (month) => Number(month.value) <= maxMonth,
  );
  ...
  function handleBirthDatePartChange(
    part: "year" | "month" | "day",
    value: string,
  ) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (typeof parsed.currentStep === "number") {
        setCurrentStep(
          Math.min(Math.max(parsed.currentStep, 0), stepConfigs.length - 1),
        );
      }
    } catch {
  ...
  useEffect(() => {
    ...
    const subscription = form.watch((values) => {
      window.localStorage.setItem(
        REGISTER_DRAFT_KEY,
        JSON.stringify({ values, currentStep }),
      );
    });
  ...
  useEffect(() => {
    ...
    const handler = (e: MouseEvent) => {
      if (
        allergyContainerRef.current &&
        !allergyContainerRef.current.contains(e.target as Node)
      ) {
  ...
  useEffect(() => {
    void getCompaniesListForDropdown()
  ...
  useEffect(() => {
    if (watchedCompanyId && companiesList.length > 0) {
      const match = companiesList.find((c) => c.id === watchedCompanyId);
      if (match && selectedCompanyName !== match.companyName) {
        setSelectedCompanyName(match.companyName);
        setCompanySearch(match.companyName);
      }
    }
  }, [watchedCompanyId, companiesList, selectedCompanyName]);
  ...
  const allergySuggestions = allergyOptions
    ...
    .filter(
      (opt) =>
        allergyInput === "" ||
        opt.toLowerCase().includes(allergyInput.toLowerCase()),
    );
    ...
    <>
              ...
              <div className="relative w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl">
                ...
                <div className="flex justify-center pt-12 pb-6">
                  ...
                  >
                    ...
                    {/* Envelope flap */}
                    <path
                      d="M8 36L60 68L112 36"
                      stroke="white"
                      strokeWidth="2"
                      fill="none"
                    />
                    <path
                      d="M8 32L60 64L112 32L112 36L60 68L8 36Z"
                      fill="var(--color-primary)"
                    />
                    {/* Shadow line */}
                    <rect
      ...
      <header className="mb-10">
        ...
        <p className="mt-2.5 max-w-lg text-sm leading-relaxed text-slate-500">
          Add your details once so we can book visits and reach you when it
          matters. You can update anything later from your profile.
        </p>
      </header>
      ...
      <form className="space-y-8" onSubmit={onSubmit} noValidate>
        <div className="space-y-2">
          <div className="flex items-center justify-between text-xs font-medium text-slate-500">
            ...
            <span>
              {Math.round(((currentStep + 1) / stepConfigs.length) * 100)}%
            </span>
          </div>
        ...
        {currentStep === 1 ? (
          <section className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <FormField error={form.formState.errors.companyId?.message}>
                  ...
                  <p className="text-xs text-slate-500">
                    Select your employer or company if applicable.
                  </p>
                </FormField>
              </div>
              <div className="sm:col-span-2">
            ...
            <div className="space-y-4">
              ...
              <FormField error={undefined}>
                ...
                <Textarea
                  ...
                  placeholder="Surgeries, chronic conditions, medications — or leave blank"
                />
              </FormField>
            </div>
          </section>
...
function Label({
  ...
  return (
    <label
      className={`text-sm font-medium text-slate-700 ${className}`.trim()}
      {...props}
    />
  );
}
